...
class Config:
    ...
    #cls意为这个类本身；self相当于这个类的一个实例
    #__new__函数的返回值是self，即类的一个实例
    def __new__(cls):
        if cls._instance is None:
            ...
            if hasattr(Config().trainer, 'max_concurrency'):
                ...
                # trainers
                ######
                #这里加了一个参数，check_same_thread=False
                ######
                Config.sql_connection = sqlite3.connect(
                    "/tmp/running_trainers.sqlitedb",check_same_thread=False)
